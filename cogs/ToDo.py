...
class Todo(commands.Cog):
    ...
    async def todo(self, ctx):
        ...
        Example: !todo add Get Daily Credits | 24h1m | text=t!daily | channel=299431230984683520 | repeat=yes | alert=off
            ...
            if not self.todo_list:
                ...
                for count, embed in enumerate(all_entries):
                    if len(all_entries) > 1:
                        embed.title = "{}'s to-do list ({}/{}):".format(ctx.message.author.name.format(), count + 1,
                                                                        len(all_entries))
